-- ============================================================
-- LeMiCi IQ — Complete PostgreSQL Schema
-- Date: 2026-06-30
-- Version: 6.0
-- FLE prerequisite: email citext→TEXT, chk_email_format removed, column sizes expanded, archived status added
-- Source: 30-schema.sql + IMPLEMENTATION_PLAN_User_Profile.md
-- ============================================================
-- This file is the single source of truth for the database schema.
-- It supersedes deployments/docker/postgres/30-schema.sql.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS users (
    ...
    email           TEXT        NOT NULL,                                              -- FLE: citext→TEXT; ciphertext is case-sensitive base64
    email_verified  boolean     NOT NULL DEFAULT false,
    status          text        NOT NULL DEFAULT 'active',
    name            VARCHAR(500),                                                      -- FLE: expanded for AES-256-GCM + base64 ciphertext
    phone           VARCHAR(100),                                                      -- FLE: expanded for AES-256-GCM + base64 ciphertext
    phone_verified  boolean     DEFAULT false,
    location        varchar(255),
    profile_image   varchar(500),
    archived_at     TIMESTAMPTZ DEFAULT NULL,                                          -- DPDPA: set on account archival
    created_at      timestamptz NOT NULL DEFAULT NOW(),
    updated_at      timestamptz NOT NULL DEFAULT NOW(),

    CONSTRAINT users_email_unique UNIQUE (email),
    -- REMOVED: chk_email_format — email validated at app layer before FLE encryption
...
CREATE TABLE franchise_shares (
    ...
    retain_until TIMESTAMP DEFAULT (NOW() + INTERVAL '365 days'),                     -- DPDPA: IP retention limit
...
CREATE TABLE notifications (
    ...
    retain_until TIMESTAMP DEFAULT (NOW() + INTERVAL '180 days'),                      -- DPDPA: notification retention limit
...
CREATE TABLE IF NOT EXISTS contact_messages (
    ...
    retain_until TIMESTAMP WITH TIME ZONE DEFAULT (NOW() + INTERVAL '90 days')         -- DPDPA: anonymous PII retention limit
);
...
CREATE TABLE IF NOT EXISTS public_form_submissions (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    retain_until TIMESTAMP DEFAULT (NOW() + INTERVAL '90 days')                        -- DPDPA: anonymous PII retention limit
);
...
CREATE TABLE IF NOT EXISTS user_professional_details (
    ...
    created_at        TIMESTAMPTZ DEFAULT NOW(),
    updated_at        TIMESTAMPTZ DEFAULT NOW()
    -- No CHECK constraints: validated at app layer via config/dropdowns.yaml
);
...
CREATE TABLE IF NOT EXISTS user_company_details (
    user_id              UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
    business_name        VARCHAR(500),                  -- FLE: expanded for ciphertext
    business_type        VARCHAR(100),
    industry_sector      VARCHAR(255),
    year_established     INTEGER,
    cin_registration     VARCHAR(100),                  -- FLE: expanded for ciphertext
    gst_number           VARCHAR(100),                  -- FLE: expanded for ciphertext
    annual_turnover      VARCHAR(50),
    company_website      VARCHAR(500),
    company_phone        VARCHAR(100),                  -- FLE: expanded for ciphertext
...
CREATE TABLE IF NOT EXISTS profile_audit_log (
    ...
    old_value   TEXT,
    new_value   TEXT,
    changes     JSONB,                                                      -- Full submitted delta blob from BPMN
    source      VARCHAR(50),
...
CREATE TABLE IF NOT EXISTS user_consents (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    consent_type    VARCHAR(100) NOT NULL,                   -- e.g. 'privacy_policy', 'marketing_email', 'sms_notifications'
    version         VARCHAR(50) NOT NULL,                    -- e.g. 'v2.1' — version of policy/terms consented to
    granted         BOOLEAN NOT NULL DEFAULT true,           -- true = granted, false = withdrawn
    granted_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    withdrawn_at    TIMESTAMPTZ,                             -- NULL if still active
    ip_address      VARCHAR(45),                             -- IPv4/IPv6 of consent origin
    source          VARCHAR(50) NOT NULL,                    -- e.g. 'signup', 'settings', 'onboarding'
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Sessions are Redis-only, no SQL table. Schema: session:{sessionId} → {userId, claims, expiry}
...
-- Cleanup job for expired guest audit events
...
COMMENT ON FUNCTION cleanup_expired_share_ips() IS
    'Anonymizes IP addresses in franchise_shares past their 365-day retention period. Run via cron job weekly. DPDPA compliance.';
