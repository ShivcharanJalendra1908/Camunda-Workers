...
(async () => {
  ...
  if (args.length < 2) {
    console.error('Usage: node gen_pdf_generic.js <input_markdown_path> <output_pdf_path> [title]');
    process.exit(1);
  }

  const mdPath = path.resolve(args[0]);
  const pdfPath = path.resolve(args[1]);
  const docTitle = args[2] || 'LeMiCi API Specification';
...
<head>
  ...
  <style>
    ...
    .mermaid svg {
      ...
      max-height: 380px !important; /* Cap vertical scaling of diagrams */
    }
...
<body class="markdown-body">
  ...
  <script>
    ...
    // Set a wide viewport to prevent wrapping
    await page.setViewport({ width: 1200, height: 900 });
    
    page.setDefaultNavigationTimeout(0);
    await page.setContent(htmlTemplate, { waitUntil: 'domcontentloaded', timeout: 0 });
    
    console.log('Waiting for Mermaid diagrams to render (if any)...');
    ...
    await page.pdf({
      ...
      headerTemplate: `<div style="font-size: 8px; font-family: -apple-system, sans-serif; width: 100%; text-align: right; padding-right: 25px; color: #888;">\${docTitle}</div>`,
