...
func (h *WorkflowHandler) StartKeycloakLogout(c *gin.Context) {
	...
	// ✅ variables pehle banao, PHIR correlationKey add karo
	variables := map[string]interface{}{
		"sessionId":      sessionID,
		"userId":         getOrDefault(input.UserID, claims.UserID),
		"logoutAll":      input.LogoutAll,
		...
		"sourceSystem":   claims.SourceSystem,
		"requestId":      uuid.New().String(),
		"correlationKey": correlationKey, // ✅ yahan add karo
	}
	...
	// ✅ Login ki tarah Receive() NAHI karna — directly workflow start karo
	h.startWorkflow(ctx, "keycloak-logout-workflow", variables)
