...
const TaskType = "auth.signup.linkedin"
...
func NewHandler(opts HandlerOptions) (*Handler, error) {
	...
	if err := workerConfig.Validate(); err != nil {
		return nil, fmt.Errorf("invalid configuration for auth-signup-linkedin: %w", err)
	}
...
func (h *Handler) Handle(client worker.JobClient, job entities.Job) {
	...
	h.logger.Info("Processing LinkedIn signup request", map[string]interface{}{
		"jobKey":             job.GetKey(),
	...
	if !h.rateLimiter.Allow() {
		span.RecordError(errors.NewRateLimitExceededError("linkedin_api", 100))
		span.SetAttributes(attribute.Bool("rate_limit_exceeded", true))
		h.logger.Warn("Rate limit exceeded for LinkedIn signup", map[string]interface{}{
			"worker":  TaskType,
			"traceId": traceID,
		})
		h.errorHandler.HandleJobError(ctx, client, job, errors.NewRateLimitExceededError("linkedin_api", 100))
		return
	}
	...
	_, spanParse := otel.Tracer("worker-manager").Start(ctx, "auth-signup-linkedin.parseInput")
	...
	_, spanSanitize := otel.Tracer("worker-manager").Start(ctx, "auth-signup-linkedin.sanitizeInput")
	input.Sanitize()
	spanSanitize.End()

	// ===== STEP 3: VALIDATE INPUT =====
	_, spanValidate := otel.Tracer("worker-manager").Start(ctx, "auth-signup-linkedin.validateInput")
	...
	if h.idempotencyChecker != nil {
		idempotencyKey := h.keyGenerator.GenerateSignupKey(input.Email, "linkedin")
	...
	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "auth-signup-linkedin.Execute")
	...
	ctxComp, spanComp := otel.Tracer("worker-manager").Start(ctx, "auth-signup-linkedin.completeJob")
...
func (h *Handler) completeJob(ctx context.Context, client worker.JobClient, job entities.Job, output *Output) {
	...
	if err != nil {
		...
		h.logger.Info("Successfully completed LinkedIn signup", map[string]interface{}{
			"jobKey":        job.GetKey(),
			"userId":        output.UserID,
...
func (h *Handler) Register() error {
	...
	h.logger.Info("LinkedIn signup worker registered with Camunda", map[string]interface{}{
		"taskType":      TaskType,
		"maxJobsActive": h.config.MaxJobsActive,
...
func createConfigFromAppConfig(appConfig *config.Config, customConfig *Config) *Config {
	...
	if appConfig != nil {
		...
		if workerCfg, exists := appConfig.Workers["auth-signup-linkedin"]; exists {
...
// 		// Rate limit: 100 calls per minute (approx 1.66 calls/sec), burst 10
// 		rateLimiter:  ratelimit.New(100.0/60.0, 10),
// 		errorHandler: errors.NewErrorHandler(loggerInstance),
// 	}
