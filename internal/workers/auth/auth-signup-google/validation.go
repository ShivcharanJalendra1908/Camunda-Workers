...
// ValidateInput performs comprehensive validation for auth-signup-google
...
func (input *Input) Sanitize() {
	...
	if input.Email != "" {
		...
		input.Email = sanitizer.SanitizeString(strings.ToLower(input.Email))
	}
	if input.RedirectURI != "" {
...
func GetInputSchema() validation.JSONSchema {
	return validation.JSONSchema{
		...
		Properties: map[string]validation.Property{
			...
			"firstName": {
				Type:        "string",
				Description: "User's first name (optional, will use Google profile if not provided)",
				MaxLength:   intPtr(100),
			},
			"lastName": {
				Type:        "string",
				Description: "User's last name (optional, will use Google profile if not provided)",
				MaxLength:   intPtr(100),
			},
			"metadata": {
				Type:        "object",
...
func GetOutputSchema() validation.JSONSchema {
	return validation.JSONSchema{
		...
		Properties: map[string]validation.Property{
			"success": {
				Type:        "boolean",
				Description: "Whether the Google signup was successful",
			},
			"userId": {
				Type:        "string",
