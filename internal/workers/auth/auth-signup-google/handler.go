...
const TaskType = "auth.signup.google"
...
func NewHandler(opts HandlerOptions) (*Handler, error) {
	...
	if err := workerConfig.Validate(); err != nil {
		return nil, fmt.Errorf("invalid configuration for auth-signup-google: %w", err)
	}
	...
	handler := &Handler{
		...
		rateLimiter:        ratelimit.New(1000.0/60.0, 50),
...
func (h *Handler) generateIdempotencyKey(input *Input) string {
	// Key format: email + provider + date (within same hour)
	hourWindow := time.Now().Format("2006-01-02-15") // Same hour
	data := fmt.Sprintf("signup:google:%s:%s", input.Email, hourWindow)
...
func (h *Handler) Handle(client worker.JobClient, job entities.Job) {
	...
	h.logger.Info("Processing Google signup request", map[string]interface{}{
		"jobKey":             job.GetKey(),
	...
	if !h.rateLimiter.Allow() {
		span.RecordError(errors.NewRateLimitExceededError("Google API", 1000))
		span.SetAttributes(attribute.Bool("rate_limit_exceeded", true))
		h.logger.Warn("Rate limit exceeded for Google signup", map[string]interface{}{
			"worker":  TaskType,
			"traceId": traceID,
		})
		h.errorHandler.HandleJobError(ctx, client, job, errors.NewRateLimitExceededError("Google API", 1000))
		return
	}
	...
	_, spanParse := otel.Tracer("worker-manager").Start(ctx, "auth-signup-google.parseInput")
	...
	_, spanSanitize := otel.Tracer("worker-manager").Start(ctx, "auth-signup-google.sanitizeInput")
	input.Sanitize()
	spanSanitize.End()

	// ===== STEP 3: VALIDATE INPUT =====
	_, spanValidate := otel.Tracer("worker-manager").Start(ctx, "auth-signup-google.validateInput")
	...
	if h.idempotencyChecker != nil {
		idempotencyKey := h.keyGenerator.GenerateSignupKey(input.Email, "google")

		h.logger.Debug("Checking signup idempotency", map[string]interface{}{
		...
		if err == idempotency.ErrDuplicateRequest {
			...
			h.logger.Info("Duplicate signup detected, returning cached result", map[string]interface{}{
				"idempotencyKey": idempotencyKey,
				"email":          input.Email,
				"traceId":        traceID,
			})
		...
		if err == idempotency.ErrProcessing {
			...
			h.logger.Warn("Signup already being processed", map[string]interface{}{
				"idempotencyKey": idempotencyKey,
				"traceId":        traceID,
			})
			return
	...
	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "auth-signup-google.Execute")
	...
	ctxComp, spanComp := otel.Tracer("worker-manager").Start(ctx, "auth-signup-google.completeJob")
...
func (h *Handler) parseInput(job entities.Job) (*Input, error) {
	...
	if firstName, ok := variables["firstName"].(string); ok {
		input.FirstName = firstName
	}

	if lastName, ok := variables["lastName"].(string); ok {
...
func (h *Handler) completeJob(ctx context.Context, client worker.JobClient, job entities.Job, output *Output) {
	...
	if err != nil {
		...
		h.logger.Info("Successfully completed Google signup", map[string]interface{}{
			"jobKey":        job.GetKey(),
			"userId":        output.UserID,
...
func (h *Handler) Register() error {
	...
	h.logger.Info("Google signup worker registered with Camunda", map[string]interface{}{
		"taskType":      TaskType,
		"maxJobsActive": h.config.MaxJobsActive,
...
func createConfigFromAppConfig(appConfig *config.Config, customConfig *Config) *Config {
	...
	if appConfig != nil {
		googleConfig := appConfig.Auth.OAuthProviders.Google
		if googleConfig.ClientID != "" {
			cfg.ClientID = googleConfig.ClientID
			cfg.ClientSecret = googleConfig.ClientSecret
			cfg.RedirectURL = googleConfig.RedirectURL
		}

		if workerCfg, exists := appConfig.Workers["auth-signup-google"]; exists {
		...
		// Add Zoho credentials from app config
...
// 		errorHandler: errors.NewErrorHandler(loggerInstance),
// 		// Rate limit: 1000 calls per minute (approx 16.6 calls/sec), burst 50
// 		rateLimiter: ratelimit.New(1000.0/60.0, 50),
// 	}
