...
func (s *Service) Execute(ctx context.Context, input *Input) (*Output, error) {
	...
	// 🔴 REQUIRED SAFETY CHECKS (SAME AS GOOGLE)
	if s.keycloak == nil {
	...
	if s.config == nil {
		return nil, &errors.StandardError{
			...
			Message:   "Auth signin linkedin config is missing",
			Retryable: false,
			Timestamp: time.Now(),
		}
	}

	s.logger.Info("Executing LinkedIn signin flow", map[string]interface{}{
		"hasRedirectURI": input.RedirectURI != "",
		"hasState":       input.State != "",
	})

	// CRITICAL: Exchange code with Keycloak (not LinkedIn directly)
	...
	s.logger.Info("LinkedIn signin completed successfully", map[string]interface{}{
		"userId":            user.ID,
		"email":             user.Email,
		"isNewUser":         isNewUser,
...
func (s *Service) validateUserInfo(userInfo *auth.TokenInfo) error {
	...
	// Note: LinkedIn doesn't always provide email_verified field
	// So we don't require it to be verified like Google
	return nil
}
