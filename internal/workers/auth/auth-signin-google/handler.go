...
const TaskType = "auth.signin.google"
...
func (h *Handler) Handle(client worker.JobClient, job entities.Job) {
	...
	h.logger.Info("Processing Google signin request", map[string]interface{}{
		"jobKey":             job.GetKey(),
	...
	if !h.rateLimiter.Allow() {
		...
		h.logger.Warn("Rate limit exceeded for Google signin", map[string]interface{}{
			"worker":  TaskType,
			"traceId": traceID,
		})
	...
	if !h.config.Enabled {
		span.SetAttributes(attribute.Bool("worker_disabled", true))
	...
	_, spanParse := otel.Tracer("worker-manager").Start(ctx, "auth-signin-google.parseInput")
	...
	_, spanSanitize := otel.Tracer("worker-manager").Start(ctx, "auth-signin-google.sanitizeInput")
	input.Sanitize()
	spanSanitize.End()

	// ===== STEP 3: VALIDATE INPUT =====
	_, spanValidate := otel.Tracer("worker-manager").Start(ctx, "auth-signin-google.validateInput")
	...
	if h.idempotencyChecker != nil {
		...
		idempotencyKey := h.keyGenerator.GenerateSigninKey(input.AuthCode, "google")
		...
		if err == idempotency.ErrDuplicateRequest {
			span.SetAttributes(attribute.Bool("idempotent_request", true))
		...
		if err == idempotency.ErrProcessing {
			span.SetAttributes(attribute.Bool("request_processing", true))
	...
	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "auth-signin-google.Execute")
	...
	ctxComp, spanComp := otel.Tracer("worker-manager").Start(ctx, "auth-signin-google.completeJob")
...
func (h *Handler) completeJobSimple(client worker.JobClient, job entities.Job, output *Output) {
	ctx := context.Background()
...
func (h *Handler) Register() error {
	...
	h.logger.Info("Google signin worker registered with Camunda", map[string]interface{}{
		"taskType":      TaskType,
		"maxJobsActive": h.config.MaxJobsActive,
...
func createConfigFromAppConfig(appConfig *config.Config, customConfig *Config) *Config {
	...
	if appConfig != nil {
		...
		if workerCfg, exists := appConfig.Workers["auth-signin-google"]; exists {
