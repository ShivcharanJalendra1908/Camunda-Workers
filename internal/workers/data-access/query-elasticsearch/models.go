...
// type Input struct {
// 	// New format - registry based
// 	QueryType models.QueryType       `json:"queryType"`
// 	Params    map[string]interface{} `json:"params"` // Query parameters

// 	// Old format - backward compatibility
// 	IndexName string                 `json:"indexName"` // e.g., "franchise_listings"
// 	Query     map[string]interface{} `json:"query"`     // Raw ES query
...
// 		ozzo.Field(&i.IndexName,
// 			ozzo.When(string(i.QueryType) == "", ozzo.Required), // Required only when not using QueryType
// 			ozzo.Length(1, 100),
// 			validation.SafeSQLString),
// 		ozzo.Field(&i.QueryType,
// 			ozzo.When(i.IndexName == "", ozzo.Required), // Required only when not using IndexName
// 			ozzo.Length(1, 100)),
// 		ozzo.Field(&i.FranchiseID, ozzo.When(i.FranchiseID != "", validation.IsUUID)),
// 		ozzo.Field(&i.Category, ozzo.Length(0, 100), validation.SafeSQLString),
// 		ozzo.Field(&i.Pagination),
// 		ozzo.Field(&i.Filters, ozzo.By(validateFilters)),
// 	)
// }

// func (i *Input) Sanitize() {
// 	i.IndexName = validation.SanitizeString(i.IndexName)
// 	i.QueryType = models.QueryType(validation.SanitizeString(string(i.QueryType)))
// 	i.FranchiseID = validation.SanitizeString(i.FranchiseID)
// 	i.Category = validation.SanitizeString(i.Category)
// }

// func validateFilters(value interface{}) error {
// 	filters, ok := value.(map[string]interface{})
// 	if !ok {
// 		return nil // nil or not a map is valid here
// 	}

// 	// Check for malicious keys
// 	for k := range filters {
// 		if strings.Contains(strings.ToLower(k), "script") {
// 			return errors.New("script fields are not allowed")
// 		}
// 	}

// 	// Check depth/complexity
// 	if len(filters) > 20 {
// 		return errors.New("too many filter criteria (max 20)")
// 	}
