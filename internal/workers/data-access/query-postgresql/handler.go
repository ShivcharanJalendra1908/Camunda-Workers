...
// 	var input Input
// 	_, spanParse := otel.Tracer("worker-manager").Start(context.Background(), "query-postgresql.parseInput")
// 	if err := json.Unmarshal([]byte(job.Variables), &input); err != nil {
...
// 	input.Sanitize()
// 	if err := input.Validate(); err != nil {
// 		h.failJob(client, job, "VALIDATION_ERROR", fmt.Sprintf("validation failed: %v", err), 0)
// 		return
// 	}
...
// 	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "query-postgresql.Execute")
// 	output, err := h.execute(ctxExec, &input)
// 	spanExec.End()
// 	if err != nil {
