...
// 	var input Input
// 	_, spanParse := otel.Tracer("worker-manager").Start(context.Background(), "validate-application-data.parseInput")
// 	if err := json.Unmarshal([]byte(job.Variables), &input); err != nil {
// 		h.errorHandler.HandleJobError(context.Background(), client, job, appErrs.NewApplicationValidationFailedError(fmt.Sprintf("parse input: %v", err)))
// 		spanParse.End()
// 		return
// 	}
// 	spanParse.End()
...
// 	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "validate-application-data.Execute")
// 	output, err := h.execute(ctxExec, &input)
// 	spanExec.End()
// 	if err != nil {
// 		h.errorHandler.HandleJobError(ctx, client, job, appErrs.NewApplicationValidationFailedError(err.Error()))
