...
// 	var input Input
// 	_, spanParse := otel.Tracer("worker-manager").Start(context.Background(), "create-application-record.parseInput")
// 	if err := json.Unmarshal([]byte(job.Variables), &input); err != nil {
...
// 	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "create-application-record.Execute")
