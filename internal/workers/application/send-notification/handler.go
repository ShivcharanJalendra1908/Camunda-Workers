...
// 	var input Input
// 	_, spanParse := otel.Tracer("worker-manager").Start(context.Background(), "send-notification.parseInput")
// 	if err := json.Unmarshal([]byte(job.Variables), &input); err != nil {
...
// 	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "send-notification.Execute")
// 	output, err := h.execute(ctxExec, &input)
// 	spanExec.End()
// 	if err != nil {
// 		errorCode := "NOTIFICATION_SEND_FAILED"
