...
// 	var input Input
// 	_, spanParse := otel.Tracer("worker-manager").Start(context.Background(), "parse-user-intent.parseInput")
// 	if err := json.Unmarshal([]byte(job.Variables), &input); err != nil {
// 		h.errorHandler.HandleJobError(context.Background(), client, job, appErrs.NewBusinessRuleError("Input parsing failed", err.Error()))
// 		spanParse.End()
// 		return
// 	}
// 	spanParse.End()
...
// 	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "parse-user-intent.Execute")
// 	res, err := h.cb.Execute(func() (interface{}, error) {
...
// 	if err != nil {
// 		if errors.Is(err, circuitbreaker.ErrCircuitOpen) {
// 			h.logger.Warn("Circuit breaker open for GenAI service, using fallback", nil)
// 			// Fallback: Return 'search' intent so user isn't blocked
// 			fallbackOutput := &Output{
// 				IntentAnalysis: IntentAnalysis{
// 					PrimaryIntent: "search",
...
// 			}
// 			_, spanCompFb := otel.Tracer("worker-manager").Start(ctx, "parse-user-intent.completeJob.fallback")
// 			h.completeJob(client, job, fallbackOutput)
// 			spanCompFb.End()
// 			return
// 		}
...
// 	_, spanComp := otel.Tracer("worker-manager").Start(ctx, "parse-user-intent.completeJob")
// 	h.completeJob(client, job, res.(*Output))
// 	spanComp.End()
// }
