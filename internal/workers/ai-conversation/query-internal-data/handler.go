...
// 	var input Input
// 	_, spanParse := otel.Tracer("worker-manager").Start(context.Background(), "query-internal-data.parseInput")
// 	if err := json.Unmarshal([]byte(job.Variables), &input); err != nil {
...
// 	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "query-internal-data.Execute")
// 	output, err := h.execute(ctxExec, &input)
// 	spanExec.End()
// 	if err != nil {
// 		var stdErr *appErrs.StandardError
