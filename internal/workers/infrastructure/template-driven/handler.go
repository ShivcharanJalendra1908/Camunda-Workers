...
import (
	...
	"camunda-workers/internal/common/validation" // ✅ ADDED (GAP #1)

	"github.com/camunda/zeebe/clients/go/v8/pkg/entities"
	"github.com/camunda/zeebe/clients/go/v8/pkg/worker"
	ozzo "github.com/go-ozzo/ozzo-validation/v4" // ✅ ADDED (GAP #1)
...
type Handler struct {
	...
	validator    *validation.Validator // ✅ ADDED (GAP #1)
	sanitizer    *validation.Sanitizer // ✅ ADDED (GAP #1)
}
...
func NewHandler(cfg *Config, log logger.Logger) (*Handler, error) {
	...
	return &Handler{
		...
		validator:    validation.NewValidator(), // ✅ ADDED (GAP #1)
		sanitizer:    validation.NewSanitizer(), // ✅ ADDED (GAP #1)
	}, nil
}
...
func (h *Handler) Handle(client worker.JobClient, job entities.Job) {
	...
	_, spanParse := otel.Tracer("worker-manager").Start(ctx, "template-driven.parseInput")
	if err := json.Unmarshal([]byte(job.Variables), &input); err != nil {
		span.RecordError(err)
		span.SetAttributes(attribute.Bool("error", true))
		h.failJobWithError(ctx, client, job, correlationID, "failed to parse input", err)
		spanParse.End()
		return
	}
	spanParse.End()

	// ===== CRITICAL: COMPREHENSIVE INPUT VALIDATION (GAP #1 FIX) =====
	ctxValidate, spanValidate := otel.Tracer("worker-manager").Start(ctx, "template-driven.validateInput")
	if err := h.validateInputComprehensive(&input); err != nil {
		span.RecordError(err)
		span.SetAttributes(attribute.Bool("error", true))
		h.errorHandler.HandleJobError(ctxValidate, client, job, err)
		spanValidate.End()
		return
	}
	spanValidate.End()
	...
	if err != nil {
		span.RecordError(err)
		span.SetAttributes(attribute.Bool("error", true))
		h.failJobWithError(ctx, client, job, correlationID, "failed to load template", err)
		return
	}
	...
	ctxExec, spanExec := otel.Tracer("worker-manager").Start(ctx, "template-driven.Execute")
	result, err := h.executeTemplateSecure(ctxExec, template, input.InputData, correlationID)
	spanExec.End()
	if err != nil {
		span.RecordError(err)
		span.SetAttributes(attribute.Bool("error", true))
		h.failJobWithError(ctx, client, job, correlationID, "execution failed", err)
		return
	}
	...
	// Complete job
	_, spanComp := otel.Tracer("worker-manager").Start(ctx, "template-driven.completeJob")
	request, err := client.NewCompleteJobCommand().JobKey(job.Key).VariablesFromObject(output)
	if err != nil {
		span.RecordError(err)
		span.SetAttributes(attribute.Bool("error", true))
		spanComp.End()
		h.failJobWithError(ctx, client, job, correlationID, "failed to create complete command", err)
		return
	}
	...
	if _, err := request.Send(ctx); err != nil {
		...
		span.SetAttributes(attribute.Bool("error", true))
		spanComp.End()
		h.failJobWithError(ctx, client, job, correlationID, "failed to send completion", err)
		return
	}
	spanComp.End()
...
func (h *Handler) validateInputComprehensive(input *InputVariables) error {
	if input == nil {
		return errors.NewValidationError("input", "input is nil")
	}

	// ✅ Use ozzo for structured validation
	if err := ozzo.ValidateStruct(input,
		ozzo.Field(&input.TemplatePath,
			ozzo.Required.Error("templatePath is required"),
			ozzo.Length(1, 500).Error("templatePath must be 1-500 characters"),
			ozzo.By(h.validateTemplatePathRule()),
		),
		ozzo.Field(&input.Format,
			ozzo.Required.Error("format is required"),
			ozzo.In("json", "xml").Error("format must be 'json' or 'xml'"),
			validation.SafeSQLString,
		),
	); err != nil {
		return errors.NewValidationError("input", err.Error())
	}

	// ✅ Validate TemplatePath with security checks
	if err := h.validateTemplatePathSecurity(input.TemplatePath); err != nil {
		return errors.NewValidationError("templatePath", err.Error())
	}

	// ✅ Validate InputData if present
	if input.InputData != nil {
		if err := h.validateInputDataSecurity(input.InputData); err != nil {
			return err
		}
	}

	// ✅ Original size validations
	if len(input.TemplatePath) > h.config.MaxPathLength {
		return errors.NewValidationError("templatePath",
			fmt.Sprintf("template path too long: %d > %d", len(input.TemplatePath), h.config.MaxPathLength))
	}

	dataSize := estimateSize(input.InputData)
	if dataSize > h.config.MaxInputSize {
		return errors.NewValidationError("inputData",
			fmt.Sprintf("input data too large: %d > %d bytes", dataSize, h.config.MaxInputSize))
	}

	if err := validateNestingDepth(input.InputData, h.config.MaxNestingDepth); err != nil {
		return errors.NewValidationError("inputData", err.Error())
	}

	return nil
}

// ===== OZZO VALIDATION RULE FOR TEMPLATE PATH =====
func (h *Handler) validateTemplatePathRule() ozzo.RuleFunc {
	return func(value interface{}) error {
		...
		for _, char := range unsafeChars {
			if strings.Contains(path, char) {
				return fmt.Errorf("contains unsafe character: %s", char)
			}
		}
		...
		fullPath := filepath.Join(h.config.TemplatesBaseDir, path)
...
// ===== SECURITY VALIDATION FOR TEMPLATE PATH =====
func (h *Handler) validateTemplatePathSecurity(path string) error {
	// 🔒 Additional SQL injection check
	if err := ozzo.Validate(path,
		validation.SafeSQLString,
	); err != nil {
		return fmt.Errorf("contains SQL injection patterns")
	}
	...
	if err := ozzo.Validate(path,
		...
		return fmt.Errorf("contains NoSQL injection patterns")
	}

	// 🔒 Script injection check
	if strings.Contains(strings.ToLower(path), "<script") ||
		strings.Contains(strings.ToLower(path), "javascript:") ||
		strings.Contains(strings.ToLower(path), "data:text/html") {
		return fmt.Errorf("contains script injection patterns")
	}
...
func (h *Handler) validateInputDataSecurity(data map[string]interface{}) error {
	// Limit number of fields
	if len(data) > 100 {
		return errors.NewArrayTooLargeError("inputData", 100, len(data))
	}
	...
	for key, value := range data {
		...
		if err := ozzo.Validate(key,
			...
			return errors.NewValidationError(fmt.Sprintf("inputData.%s", key), err.Error())
		}

		// Validate value based on type
		switch v := value.(type) {
		case string:
			if err := h.validateStringValue(key, v); err != nil {
				return err
			}
		case map[string]interface{}:
			// Recursive validation
			if err := h.validateInputDataSecurity(v); err != nil {
				return errors.NewValidationError(fmt.Sprintf("inputData.%s", key), err.Error())
			}
		case []interface{}:
			if err := h.validateArrayValue(key, v); err != nil {
				return err
			}
		case float64, int, bool, nil:
			...
			continue
		default:
			return errors.NewValidationError(fmt.Sprintf("inputData.%s", key),
...
func (h *Handler) validateStringValue(field string, value string) error {
	if len(value) > 10000 {
		return errors.NewStringTooLongError(field, 10000, len(value))
	}
	...
	if err := ozzo.Validate(value,
		...
		return errors.NewValidationError(field, fmt.Sprintf("SQL injection risk: %v", err))
	}
	...
	if err := ozzo.Validate(value,
		...
		return errors.NewValidationError(field, fmt.Sprintf("NoSQL injection risk: %v", err))
	}
	...
	for _, pattern := range dangerousPatterns {
		if strings.Contains(lower, pattern) {
			return errors.NewValidationError(field, "contains potentially unsafe script content")
		}
	}
...
func (h *Handler) validateArrayValue(field string, arr []interface{}) error {
	if len(arr) > 1000 {
		return errors.NewArrayTooLargeError(field, 1000, len(arr))
	}

	for i, item := range arr {
		switch v := item.(type) {
		case string:
			if err := h.validateStringValue(fmt.Sprintf("%s[%d]", field, i), v); err != nil {
				return err
			}
		case map[string]interface{}:
			// Limit nested object recursion
			if depth := h.getNestingDepth(v); depth > 5 {
				return errors.NewValidationError(fmt.Sprintf("%s[%d]", field, i),
...
// ===== GET NESTING DEPTH =====
func (h *Handler) getNestingDepth(data interface{}) int {
	maxDepth := 0
	var traverse func(interface{}, int)

	traverse = func(item interface{}, depth int) {
		if depth > maxDepth {
			maxDepth = depth
		}
...
// ===== KEEP ORIGINAL validateInput FUNCTION FOR BACKWARD COMPATIBILITY =====
...
func (h *Handler) loadTemplateSecure(ctx context.Context, path string, format string) (*Template, error) {
	...
	// 🔒 SANITIZE PATH FIRST (GAP #1)
	sanitizedPath := h.sanitizer.SanitizeString(path)

	// GAP FIX #1: Sanitize and validate path
	safePath, err := h.sanitizeTemplatePath(sanitizedPath)
	if err != nil {
...
func (h *Handler) executeTemplateSecure(ctx context.Context, template *Template, inputData map[string]interface{}, correlationID string) (map[string]interface{}, error) {
	// 🔒 SANITIZE INPUT DATA (GAP #1)
	sanitizedData := h.sanitizer.SanitizeInput(inputData)

	execContext := NewExecutionContext(sanitizedData)
	...
	// 🔒 SANITIZE OUTPUT DATA (GAP #1)
	return h.sanitizer.SanitizeInput(result), nil
}
...
func (h *Handler) failJobWithError(ctx context.Context, client worker.JobClient, job entities.Job, correlationID, message string, err error) {
	...
	stdErr := errors.NewTemplateValidationFailedError(fmt.Sprintf("%s: %v", message, err))
