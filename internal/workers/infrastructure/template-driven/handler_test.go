...
// 		// Try to create a symlink (this test will be skipped on Windows if symlinks aren't supported)
// 		symlinkPath := filepath.Join(handler.config.TemplatesBaseDir, "symlink.json")
// 		err := os.Symlink(regularFile, symlinkPath)
// 		if err != nil {
// 			t.Skip("Symlinks not supported on this system")
...
// 		// On Windows, symlinks might be followed automatically or require special privileges
// 		// If the file mode shows it's a regular file, skip this test as the OS is following the symlink
// 		if info.Mode().IsRegular() {
// 			t.Skip("Symlinks are automatically followed on this system, cannot test symlink rejection")
// 		}
