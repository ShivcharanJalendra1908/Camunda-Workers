	...
	tests := []struct {
	...
	}{
		{
			...
			input: Input{
				...
				Query:         "ice cream",
				Category:      "food",
				Industry:      "food", // NEW: Added industry
				Location:      "India",
		...
		},
		{
			...
			input: Input{
				...
				Category:      "education",
				Industry:      "education", // NEW: Added industry
				Location:      "India",
				MinInvestment: 50000,
		...
		},
		{
			...
			input: Input{
				...
				Query:         "footwear",
				Category:      "fashion",
				Industry:      "retail", // NEW: Added industry
				Location:      "Pan India",
		...
		// REMOVED: Space range validation since it's not implemented in the handler
		// REMOVED: Rating validation since the handler only checks if rating > 0
	}
...
func TestHandler_BuildSearchRequest(t *testing.T) {
	...
	t.Run("Education franchise search", func(t *testing.T) {
		input := &Input{
			...
			Location:  "India",
			MinRating: 4.0,
			SortBy:    "rating", // Changed from "since" to "rating" since that's what the handler supports
			SortOrder: "asc",
		}
...
func TestParseInputFunction(t *testing.T) {
	...
	t.Run("Parse complete variables", func(t *testing.T) {
		...
		assert.Equal(t, "food", input.Industry) // NEW: Check industry
	...
	t.Run("Parse with default values", func(t *testing.T) {
		...
		assert.Empty(t, input.Industry) // NEW: Check industry
...
func TestHandler_PrepareOutput(t *testing.T) {
	...
	t.Run("Prepare output", func(t *testing.T) {
		...
		assert.Equal(t, "food", output.AppliedFilters["industry"]) // NEW: Check industry
	...
	t.Run("Prepare output without industry", func(t *testing.T) {
		...
		assert.Equal(t, "food", output.AppliedFilters["category"])
		assert.Equal(t, "", output.AppliedFilters["industry"]) // Empty string for no industry
	})
}
...
func TestHandler_ConcurrentAccess(t *testing.T) {
	...
	// Test concurrent access to validateInput
