...
const (
	...
	ListingsIndex   = "franchise_listings"   // Γ£à Main search index
	HomeIndex       = "franchise_home"       // Γ£à Homepage data
	IndustriesIndex = "franchise_industries" // Γ£à Industry pages
)
...
func (h *Handler) buildSearchRequest(input *Input) (*SearchRequest, error) {
	...
	// Γ£à INDUSTRY FILTER (TOP LEVEL OBJECT)
	if input.Industry != "" {
	...
	// Γ£à LOCATION FILTER (keyword field)
	if input.Location != "" {
	...
	// Γ£à AGGREGATIONS (matching new structure)
...
func (h *Handler) executeSearch(ctx context.Context, request *SearchRequest) ([]map[string]interface{}, int, error) {
	...
	// Γ£à SEARCH franchise_listings INDEX
